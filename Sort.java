...
public class Sort {
    ...
    public void intercalacaoBalanceadaComum() throws Exception {
        ...
        while (/* fileReader.getFilePointer() < fileReader.length() */arq < 40) {
            ...
            while (currentElement < 20) {
                array[currentElement] = new Musica();
    ...
    private void setMovie(Musica m) throws Exception {

        // set if is valide
        m.setLapide(fileReader.readBoolean());

        m.setId(fileReader.readInt());

        // set movie id
        fileReader.readInt();
        m.setArtistName(fileReader.readUTF());

        fileReader.readInt();
        m.setReleaseName(fileReader.readUTF()); 

        long date
        m.setDate(fileReader.readLong());

        fileReader.readInt();
        m.setReleaseType(fileReader.readUTF());
        // read and set the rest of the movie atributes ---

        m.setReviewCount(fileReader.readInt());
        fileReader.readInt();
